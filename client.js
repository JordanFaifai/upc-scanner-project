...
let sidebar;
let sidebarOverlay;
let hamburgerMenu; // Changed from sidebarToggleButton to match HTML
...
function handleAccordionClick(event) {
    const header = event.currentTarget; // Use currentTarget to ensure the listener is on the button
    ...
    if (content.classList.contains('show')) {
        ...
        content.style.maxHeight = null; // Reset max-height to transition properly
    } else {
...
function checkDietaryCompliance(product, preferences) {
    ...
    // Check Vegan/Vegetarian (simplified example, depends on specific ingredient lists)
    if (preferences.vegan && product.vegan_status === 'not-vegan') {
        feedback.push('Not Vegan: Contains non-vegan ingredients.');
    } else if (preferences.vegan && product.vegan_status === 'potential-vegan') {
        feedback.push('Potentially Vegan: May contain animal-derived ingredients. Check carefully.');
    } else if (preferences.vegetarian && product.vegetarian_status === 'not-vegetarian') {
        feedback.push('Not Vegetarian: Contains non-vegetarian ingredients.');
    } else if (preferences.vegetarian && product.vegetarian_status === 'potential-vegetarian') {
        feedback.push('Potentially Vegetarian: May contain non-vegetarian ingredients. Check carefully.');
    }

    // Check Gluten-Free
    if (preferences.glutenFree && product.gluten_free_status === 'not-gluten-free') {
        feedback.push('Not Gluten-Free: Contains gluten.');
    } else if (preferences.glutenFree && product.gluten_free_status === 'potential-gluten-free') {
    ...
    if (preferences.allergens && preferences.allergens.length > 0) {
        preferences.allergens.forEach(allergen => {
            if (lowerCaseIngredients.includes(allergen.toLowerCase())) {
                feedback.push(`Allergen Alert: Contains "${allergen}"`);
            }
        });
    }

    // Check additives against preferences (if API provides additive risk data and you have preferences for them)
    // This part requires more complex logic based on how you define "banned" additives in preferences
    if (product.additives && product.additives.length > 0) {
        product.additives.forEach(add => {
            if (add.status && add.status.includes('BANNED in EU')) {
                feedback.push(`Additive Alert: ${add.eNumber || add.name} is BANNED in EU.`);
            }
...
async function fetchAndProcessProduct(upc, fromScan) {
    ...
    if (fromScan) {
        // Save to history only for scans, not manual lookups (to avoid duplicates if user clicks lookup after scan)
    ...
    if (product) {
        ...
        if (complianceFeedback.length > 0) {
            ...
            feedbackHtml += '</ul></div>';
            productInfoDiv.insertAdjacentHTML('afterbegin', feedbackHtml); // Add to the top of product info
        }
...
function getPerServingValue(nutrient) {
    if (nutrient && nutrient.per_serving !== undefined) {
        return parseFloat(nutrient.per_serving).toFixed(1);
    }
    return 'N/A';
}
...
function renderProductInfo(product) {
    ...
    if (product.nova_group) {
        ...
        const novaAdvice = product.nova_group_advice || 'No specific advice.';
        html += `
        ...
        html += `
            <div class="section-card">
                ...
                <div class="accordion-content hidden">
                    <p>${product.ingredients || 'No ingredients listed.'}</p>
    ...
    if (product.additives && product.additives.length > 0) {
        ...
        product.additives.forEach(add => {
            ...
            if (add.status && add.status.includes('BANNED in EU')) {
                ...
                statusClass += ' info'; // Use 'info' for other specific statuses
            } else if (add.status && (add.status === 'Unknown Status' || add.status === 'Details from Wikipedia.')) {
                statusText = 'Info limited';
                statusClass += ' info'; // Use 'info' for unknown/wikipedia statuses
            } else {
